...
export function CartDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30">
      <button type="button" aria-label="Close cart" onClick={closeCart} className="hidden flex-1 cursor-pointer sm:block" />
      <aside className="flex h-full w-[536px] max-w-full flex-col bg-white text-brand-text shadow-[-8px_0_24px_rgba(0,0,0,0.12)]">
        <div className="flex items-center justify-between px-[31px] pb-7 pt-[29px]">
          ...
          <button type="button" aria-label="Close cart" onClick={closeCart} className="cursor-pointer text-[#676869]">
            <CloseIcon />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-[31px]">
          {lines.length === 0 ? (
            ...
            lines.map((line) => (
              <div key={line.id} className="grid grid-cols-[100px_1fr_auto] gap-6 border-b border-[#eee] py-6">
                <div className="relative h-[150px] w-[100px] overflow-hidden bg-[#f0f1f3]">
                  {line.image ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={line.image} alt={line.title} className="h-full w-full object-cover object-top" />
                  ) : null}
                </div>
                <div>
                  <h3 className="text-[14px] font-bold leading-5 text-[#4d4f52]">{line.title}</h3>
                  ...
                  <div className="mt-8 inline-grid h-[35px] grid-cols-3 border border-[#d6d6d6] text-[#8c8c8c]">
                    <button type="button" disabled={pending} onClick={() => run(() => setQty(line.id, line.quantity - 1))} className="w-9 cursor-pointer disabled:opacity-40" aria-label="Reduce quantity">-</button>
                    <span className="grid w-9 place-items-center text-[13px]">{line.quantity}</span>
                    <button type="button" disabled={pending} onClick={() => run(() => setQty(line.id, line.quantity + 1))} className="w-9 cursor-pointer disabled:opacity-40" aria-label="Increase quantity">+</button>
                  </div>
                  <button type="button" disabled={pending} onClick={() => run(() => remove(line.id))} className="mt-3 block text-[11px] text-[#676869] underline disabled:opacity-40">
                    Remove
                  </button>
                </div>
        ...
        <div className="border-t border-[#e3e3e3] px-[31px] py-7">
          <div className="flex items-center justify-between text-[22px] leading-none">
            <p>Subtotal:</p>
            <p>{cart ? formatShopifyMoney(cart.subtotal) : "—"}</p>
          </div>
          <p className="mt-3 text-[12px] text-[#676869]">Taxes and shipping calculated at checkout.</p>
          <button
            type="button"
            onClick={checkout}
          ...
          <Link href="/track" onClick={closeCart} className="mt-4 block text-center text-[12px] text-[#676869] underline">
            Track an order
          </Link>
        </div>
      </aside>
